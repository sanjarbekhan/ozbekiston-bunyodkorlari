cd C:\Users\User\Desktop\ozbekiston-bunyodkorlari

# 1) Supabase maqolalarini beradigan API route yaratamiz
New-Item -ItemType Directory -Force -Path ".\src\app\api\tavsiyalar-articles" | Out-Null

@'
import { NextResponse } from "next/server";
import { supabase } from "@/lib/supabase";

export async function GET() {
  const { data, error } = await supabase
    .from("articles")
    .select("id, title, slug, category, image_url, description, created_at")
    .eq("status", "published")
    .order("created_at", { ascending: false })
    .limit(40);

  if (error) {
    return NextResponse.json({ articles: [], error: error.message }, { status: 500 });
  }

  return NextResponse.json({ articles: data || [] });
}
'@ | Set-Content ".\src\app\api\tavsiyalar-articles\route.ts" -Encoding UTF8

# 2) Tavsiyalar HTML ichidagi xato feed blokini maqolalar blokiga almashtiramiz
$file = ".\public\tilda\tavsiyalari.html"

if (!(Test-Path $file)) {
  Write-Host "Fayl topilmadi: $file"
  exit
}

$text = Get-Content $file -Raw -Encoding UTF8

Copy-Item $file ".\public\tilda\tavsiyalari.before-articles-block.html" -Force

$articlesBlock = @'
<div id="custom-tavsiyalar-articles" style="padding: 20px 18px 60px; background:#f2f2f2;">
  <style>
    #custom-tavsiyalar-articles * {
      box-sizing: border-box;
      font-family: Arial, Helvetica, sans-serif;
    }

    #custom-tavsiyalar-articles .oby-grid {
      display: grid;
      grid-template-columns: repeat(2, minmax(0, 1fr));
      gap: 16px;
      max-width: 1160px;
      margin: 0 auto;
    }

    #custom-tavsiyalar-articles .oby-card {
      display: block;
      overflow: hidden;
      background: #fff;
      text-decoration: none;
      color: #111827;
      box-shadow: 0 14px 32px rgba(0,0,0,0.08);
      transition: .25s ease;
    }

    #custom-tavsiyalar-articles .oby-card:hover {
      transform: translateY(-4px);
      box-shadow: 0 22px 45px rgba(0,0,0,0.14);
    }

    #custom-tavsiyalar-articles .oby-image-wrap {
      position: relative;
      width: 100%;
      height: 250px;
      background: #f3f3f3;
      overflow: hidden;
    }

    #custom-tavsiyalar-articles .oby-image {
      width: 100%;
      height: 100%;
      object-fit: cover;
      object-position: top;
      display: block;
    }

    #custom-tavsiyalar-articles .oby-category {
      position: absolute;
      left: 12px;
      top: 12px;
      background: #0043a4;
      color: white;
      padding: 8px 11px;
      font-size: 10px;
      font-weight: 900;
      text-transform: uppercase;
      letter-spacing: .08em;
      z-index: 2;
    }

    #custom-tavsiyalar-articles .oby-content {
      padding: 20px;
    }

    #custom-tavsiyalar-articles .oby-title {
      font-size: 22px;
      line-height: 1;
      letter-spacing: -0.04em;
      font-weight: 900;
      margin: 0;
    }

    #custom-tavsiyalar-articles .oby-desc {
      margin-top: 14px;
      color: #0043a4;
      font-size: 12px;
      line-height: 1.55;
      font-weight: 800;
      display: -webkit-box;
      -webkit-line-clamp: 4;
      -webkit-box-orient: vertical;
      overflow: hidden;
    }

    #custom-tavsiyalar-articles .oby-date {
      margin-top: 18px;
      color: #6b7280;
      font-size: 11px;
      letter-spacing: .12em;
      font-weight: 600;
    }

    #custom-tavsiyalar-articles .oby-loading {
      max-width: 1160px;
      margin: 0 auto;
      background: white;
      padding: 30px;
      text-align: center;
      font-weight: 900;
      color: #0043a4;
    }

    @media (min-width: 900px) {
      #custom-tavsiyalar-articles .oby-grid {
        grid-template-columns: repeat(4, minmax(0, 1fr));
        gap: 32px;
      }

      #custom-tavsiyalar-articles .oby-image-wrap {
        height: 290px;
      }
    }

    @media (max-width: 640px) {
      #custom-tavsiyalar-articles {
        padding: 18px 16px 45px;
      }

      #custom-tavsiyalar-articles .oby-scroll {
        display: flex;
        gap: 14px;
        overflow-x: auto;
        scroll-snap-type: x mandatory;
        padding-bottom: 12px;
      }

      #custom-tavsiyalar-articles .oby-card {
        min-width: 82%;
        scroll-snap-align: start;
      }

      #custom-tavsiyalar-articles .oby-image-wrap {
        height: auto;
      }

      #custom-tavsiyalar-articles .oby-image {
        height: auto;
        object-fit: contain;
      }
    }
  </style>

  <div class="oby-loading" id="oby-loading">Maqolalar yuklanmoqda...</div>
  <div class="oby-grid oby-scroll" id="oby-articles-grid"></div>

  <script>
    function obyCleanText(text) {
      if (!text) return "";
      return String(text).replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
    }

    function obyFirstCategory(category) {
      if (!category) return "";
      return String(category).split(";").map(function(item) {
        return item.trim();
      }).filter(Boolean)[0] || "";
    }

    function obyFormatDate(date) {
      if (!date) return "";
      var p = String(date).slice(0, 10).split("-");
      if (p.length !== 3) return "";
      return p[2] + "." + p[1] + "." + p[0];
    }

    fetch("/api/tavsiyalar-articles")
      .then(function(res) { return res.json(); })
      .then(function(data) {
        var loading = document.getElementById("oby-loading");
        var grid = document.getElementById("oby-articles-grid");
        var articles = data.articles || [];

        if (loading) loading.style.display = "none";

        if (!articles.length) {
          if (grid) {
            grid.innerHTML = '<div class="oby-loading">Maqola topilmadi</div>';
          }
          return;
        }

        grid.innerHTML = articles.map(function(article) {
          var cat = obyFirstCategory(article.category);
          var desc = obyCleanText(article.description);
          var image = article.image_url || "";
          var date = obyFormatDate(article.created_at);

          return (
            '<a class="oby-card" href="/bunyodkorlar/' + article.slug + '">' +
              '<div class="oby-image-wrap">' +
                (cat ? '<span class="oby-category">' + cat + '</span>' : '') +
                (image ? '<img class="oby-image" src="' + image + '" alt="' + article.title.replace(/"/g, '&quot;') + '">' : '') +
              '</div>' +
              '<div class="oby-content">' +
                '<h3 class="oby-title">' + article.title + '</h3>' +
                (desc ? '<p class="oby-desc">' + desc + '</p>' : '') +
                (date ? '<p class="oby-date">' + date + '</p>' : '') +
              '</div>' +
            '</a>'
          );
        }).join("");
      })
      .catch(function() {
        var loading = document.getElementById("oby-loading");
        if (loading) loading.innerText = "Maqolalarni yuklashda xatolik bo‘ldi";
      });
  </script>
</div>
'@

# Tilda feed xatosi turgan blokni topamiz va faqat o'sha blokni almashtiramiz
$marker = "ERROR:The Tilda is configured for another domain"
$index = $text.IndexOf($marker)

if ($index -lt 0) {
  $marker = "js-feed"
  $index = $text.IndexOf($marker)
}

if ($index -lt 0) {
  $marker = "t-feed"
  $index = $text.IndexOf($marker)
}

if ($index -gt 0) {
  $before = $text.Substring(0, $index)
  $recStart = $before.LastIndexOf('<div id="rec')

  if ($recStart -ge 0) {
    $nextRec = $text.IndexOf('<div id="rec', $recStart + 10)

    if ($nextRec -gt $recStart) {
      $text = $text.Substring(0, $recStart) + $articlesBlock + $text.Substring($nextRec)
      Set-Content $file $text -Encoding UTF8
      Write-Host "Tavsiyalar sahifasidagi Tilda feed xatosi o'rniga maqolalar bloki qo'shildi."
    } else {
      Write-Host "Keyingi Tilda blok topilmadi."
    }
  } else {
    Write-Host "Feed blok boshlanishi topilmadi."
  }
} else {
  Write-Host "Feed xatosi topilmadi. Balki oldin almashtirilgan bo'lishi mumkin."
}